...
BASE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "simulate_fog")
# INTEGRAL_PATH = "./simulate_fog/"
...
class ParameterSet:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.inte_path = os.path.join(BASE_PATH, kwargs.get("inte_name", "o"), "original")

        

def get_available_alphas(p: ParameterSet) -> List[float]:

    alphas = []

    for file in os.listdir(p.inte_path):
...
def get_integral_dict(p: ParameterSet) -> Dict:
    ...
    alphas = get_available_alphas(p)

    alpha = min(alphas, key=lambda x: abs(x - p.alpha))
    tau_h = min(AVAILABLE_TAU_Hs, key=lambda x: abs(x - int(p.tau_h * 1e9)))

    filename = os.path.join(p.inte_path, f'integral_0m_to_200m_stepsize_0.1m_tau_h_{tau_h}ns_alpha_{alpha}.pickle')
...
def simulate_fog(data_dict=None, config=None) -> np.ndarray:
    ...
    if data_dict is None:
        return partial(simulate_fog, config=config)
    
    noise = config.NOISE
    noise_variant = config.NOISE_VARIANT
    gain = config.GAIN
    beta = config.BETA
    ...
    parameter_set = ParameterSet(alpha=available_alpha, gamma=0.000001, beta=beta, inte_name=inte_name)
