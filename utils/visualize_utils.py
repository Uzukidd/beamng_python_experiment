...
class mayavi_animate_visualizer:
    
    def __init__(self, fig = None, max_bounding_box=30, max_ground_truth=30) -> None:
        if fig is None:
            fig = self.create_fig()
        
        self.fig = fig
        self.max_bounding_box = max_bounding_box
        self.max_ground_truth = max_ground_truth
        self.bounding_box = None
        self.point_clouds = mlab.points3d(0, 0, 0, 0, mode='point', figure=self.fig)
        
    def create_fig(self, bgcolor=(0, 0, 0), fgcolor=(1.0, 1.0, 1.0), show_intensity=False, size=(600, 600)):
        return mlab.figure(figure=None, bgcolor=bgcolor, fgcolor=fgcolor, engine=None, size=size)
    
    def update_point_clouds(self, pcs:np.ndarray):
        self.point_clouds.mlab_source.reset(x = pcs[:, 0],
                        y = pcs[:, 1],
                        z = pcs[:, 2], scalars = np.ones_like(pcs[:, 0]))
        # self.point_clouds.mlab_source.reset(x=pcs[:, 0], y=pcs[:, 1], z=pcs[:, 2], scalars=None)
        return self.fig
        
    def init_bounding_box(self):
        pass
    
    def update_bounding_box(self, boudning_boxes:list):
        pass
...
def visualize_pts(pts, fig=None, bgcolor=(0, 0, 0), fgcolor=(1.0, 1.0, 1.0),
    ...
    if show_intensity:
        ...
        G = mlab.points3d(pts[:, 0], pts[:, 1], pts[:, 2], mode='point',
                          colormap='gnuplot', scale_factor=1, figure=fig)
    if draw_origin:
        mlab.points3d(0, 0, 0, color=(1, 1, 1), mode='cube', scale_factor=0.2, figure=fig)
...
def draw_scenes_maya(fig, points, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None):
