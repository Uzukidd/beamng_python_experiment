...
def parse_arguments():
    ...
    argparser.add_argument(
        ...
        help="modeol config filename (*.yaml)",
    )
    argparser.add_argument(
        "-ckpt",
        "--model-ckpt",
        default="modelzoo/pointpillar_7728.pth",
        help="model checkpoint filename (*.pth)",
    )
    argparser.add_argument(
        "-v",
        "--preview",
...
def main(args):
    ...
    pcs_dataset = ros_point_cloud_dataset(
    ...
    try:
        cfg_from_yaml_file(args.model_config, cfg)
        model = build_network(
            model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=pcs_dataset
        )
        model.load_params_from_file(
            filename=args.model_ckpt, logger=logger, to_cpu=True
        )
        model.cuda()
        model.eval()
    except Exception as e:
